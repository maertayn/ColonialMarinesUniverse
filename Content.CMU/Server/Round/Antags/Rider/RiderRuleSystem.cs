...
public sealed partial class RiderRuleSystem : EntitySystem
{
    ...
    private void OnAntagSelected(Entity<RiderRuleComponent> ent, ref AfterAntagEntitySelectedEvent args)
    {
        ...
        var spawn = FindIsolatedVentSpawn(crew);
        var hatchling = Spawn(HatchlingPrototype, spawn);
    ...
    /// <summary>
    /// Picks the vent with the largest distance to the nearest player: no
    /// witnesses at spawn, and emerging from the vent stays the player's call.
    /// </summary>
    private EntityCoordinates FindIsolatedVentSpawn(EntityUid fallback)
    {
        var fallbackCoords = Transform(fallback).Coordinates;
        EntityCoordinates? best = null;
        var bestDistance = -1f;

        var ventEnumerator = EntityQueryEnumerator<VentCrawlableComponent, TransformComponent>();
        while (ventEnumerator.MoveNext(out _, out var ventXform))
        {
            var ventPos = ventXform.Coordinates;
