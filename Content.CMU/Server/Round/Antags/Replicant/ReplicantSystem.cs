...
public sealed partial class ReplicantSystem : EntitySystem
{
    ...
    private void OnTransformDoAfter(Entity<ReplicantComponent> ent, ref ReplicantTransformDoAfterEvent args)
    {
        ...
        SyncTransponder(ent, name);
        _popup.PopupEntity(Loc.GetString("replicant-transform-finish", ("name", name)), ent, ent);
    }

    private void SyncTransponder(EntityUid replicant, string name)
    {
...
/// <summary>
/// A programmable fake ID. Copies the assumed identity's name on transform; grants no access,
/// the real thing has to be taken off the original.
/// </summary>
[RegisterComponent]
